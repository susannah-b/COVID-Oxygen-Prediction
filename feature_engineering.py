...
sample_inves_2=True  # todo also commit to see changed when working. and quant_samples is removed so djust accordingly
...
# Drop the columns from Surrey that have been filtered for missingness in ISARC
if not validate and validation_compatible:
    isaric_final_path = Path("./validation_data/ISARIC_final.csv")
    if not isaric_final_path.exists():
        print(f"Error: Validation file '{isaric_final_path}' not found. Due to the preprocessing required to convert the "
              f"ISARIC column names to the same as Surrey, for now the validation set must be run first. Set validate to "
              f"True and rerun the script.", file=sys.stderr)
        sys.exit(1)
    isaric_final = pd.read_csv(isaric_final_path)
    isaric_columns = set(isaric_final.columns)
    extra_cols = [col for col in merged.columns if col not in isaric_columns]
    if extra_cols:
        print(f"Dropping {len(extra_cols)} extra columns not in the validation set:")
        for col in extra_cols:
            print(f"  - {col}")
        # Drop them
        merged = merged.drop(columns=extra_cols)
    else:
        print("No extra columns to drop. `merged` already matches reference schema.")

# TODO: The extra columns should also be droppped from ISARIC that aren't in Surrey so as not to affect imputation/scaling
#  This is skipped for now but as mentioned, the script needs to be converted to functions so both processing can be done
#  simultaneously.
...
# TODO should be converted to functions/classes instead of the current if/else system for validation. Then the dropping
#  of missing columns (from both datasets) could be completed without having to run the script twice for 1) validation
#  and 2) training data.